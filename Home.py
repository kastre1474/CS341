...
def init(root, user = None, conn = None):
    if(user == None):
        user = User.user(None, None, None, None, None, 0, None)
        conn = Conn.conn()
    else:
        user = user
        conn = conn
...
def setup(root, user):
    ...
    root.bind("<Escape>", lambda: exitFullScreen(root))
    root.bind("<F1>", lambda: enterFullScreen(root))
